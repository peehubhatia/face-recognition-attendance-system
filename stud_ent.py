...
class Student:
    def __init__ (self,root):
        ...
        #top center image
        img1=Image.open(r"college_images/facial-recognition_0.jpg")
        ...
        #background image
        img3=Image.open(r"college_images/facial-recognition_0.jpg")
        ...
        main_frame.place(x=0,y=50,width=1760,height=600)

        #Left Label Frame

        left_frame=LabelFrame(main_frame,bd=2,bg="white",relief=RIDGE, text="Student Details", font=("rockwell extra bold",12,"underline"))
        left_frame.place(x=0,y=10,width=750,height=480)

        #img_left=Image.open(r"C:\Users\peehu bhatia\Desktop\face reconization project\college_images\face-recognition.png")
        #img_left=img_left.resize((650,130))
        #self.photoimg_left=ImageTk.PhotoImage(img_left)

        #f_lbl=Label(left_frame,image=self.photoimg_left)
        #f_lbl.place(x=0,y=0,width=650,height=70)
        ...
        current_frame.place(x=0,y=0,width=750,height=120)
        ...
        student_frame.place(x=0,y=110,width=750,height=420)
    
        #Student Id
        id_label=Label(student_frame,text="Student ID",font=("rockwell",12), bg="white")
        id_label.grid(row=0,column=0,sticky=W)

        studentID_entry=ttk.Entry(student_frame,textvariable=self.var_id,width=20,font=("time new roman",12))
        studentID_entry.grid(row=1,column=0,sticky=W)
        ...
        gender_combo.grid(row=5,column=0,sticky=W)
        ...
        dob_entry.grid(row=5,column=1,sticky=W)
        ...
        #save Button
        save_btn=Button(student_frame,text="Save", command=self.add_data, width=17,font=("rockwell",12,"bold"),bg="dark blue",fg="white")
        save_btn.grid(row=11,column=0)

         #update Button
        update_btn=Button(student_frame,text="Update",command=self.update_data, width=8,font=("rockwell",12,"bold"),bg="dark blue",fg="white")
        update_btn.grid(row=11,column=1)

         #delete Button
        delete_btn=Button(student_frame,text="Delete",command=self.delete_data, width=17,font=("rockwell",12,"bold"),bg="dark blue",fg="white")
        delete_btn.grid(row=12,column=0)

         #reset Button
        reset_btn=Button(student_frame,text="Reset",command=self.reset_data,width=17,font=("rockwell",12,"bold"),bg="dark blue",fg="white")
        reset_btn.grid(row=12,column=1)
        ...
        photo_btn=Button(student_frame,text="Take Photo",command=self.generate_dataset, width=35,font=("rockwell",12,"bold"),bg="dark blue",fg="white")
        photo_btn.grid(row=13,column=0)

        #Update Photo Button
        up_btn=Button(student_frame,text="Update Photo",width=35,font=("rockwell",12,"bold"),bg="dark blue",fg="white")
        up_btn.grid(row=13,column=1)
        ...
        img_right=Image.open(r"college_images/facial-recognition_0.jpg")
        img_right=img_right.resize((650,130))
        self.photoimg_right=ImageTk.PhotoImage(img_right)

        r_lbl=Label(right_frame,image=self.photoimg_right)
        r_lbl.place(x=5,y=0,width=650,height=130)
        ...
        system_frame.place(x=5,y=60,width=650,height=65)
        ...
        table_frame.place(x=5,y=120,width=650,height=350)
    ...
    def add_data(self):
        if self.var_dept.get()=="Select" or self.var_name.get()=="" or self.var_id.get=="":
            ...
            try:
                conn=mysql.connector.connect(host="localhost", username="root" , password="Peehubhatia@123", database="face_recognizer",ssl_disabled=True)
...
##################   Generate Data Set or take photo sample    ################################
    ...
    def generate_dataset(self):
        if self.var_dept.get() == "Select" or self.var_name.get() == "" or self.var_id.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host="localhost", username="root", password="Peehubhatia@123",
                                           database="face_recognizer",ssl_disabled=True)
                my_cursor = conn.cursor()
                my_cursor.execute("select * from student")
                ...
                my_cursor.execute(
                    ...
                    "update student set Dept=%s,Course=%s,Year=%s,Semester=%s,Name=%s,RollNo=%s,Gender=%s,DateOfBirth=%s,ContactNo=%s, Email=%s,PWD=%s,Category=%s,Address=%s,PhotoSample=%s where StudentId=%s",
                ...
                while True:
                    ...
                    if face_cropped(myframe) is not None:
                        ...
                        file_name_path = "data/user_" + str(id) + "_" + str(img_id) + ".jpg"
                ...
                messagebox.showerror("Error", "Error Occurred", parent=self.root)
                ...
                #while True:
                   # ret, frame = cam.read()
                    ...
                    #    img_id += 1
                        ...
                        #face = cv2.cvtColor(face, cv2.COLOR_BGR2GRAY)
                    ##file_name_path = f"data/user_{id}_{img_id}.jpg"
                    #cv2.imwrite(file_name_path, face)
                     #   cv2.putText(face, str(img_id), (50, 50), cv2.FONT_HERSHEY_COMPLEX, 2, (0, 255, 0), 2)
                    ##cv2.imshow("Cropped Face", frame)#face
                        

                    ##k= cv2.waitKey(1)#== 13 or img_id == 100:
                    #3if k%256 ==27:
                        ##prnt("ghh")
                       #3333 break
                    ##elif k%256 ==32:
                      ##  data ="opencv_fram_{}.png".format()(img_counter)
    ...
    #def gerate_dataset(self):
        #if self.var_dept.get()=="Select" or self.var_name.get()=="" or self.var_id.get=="":
       #     messagebox.showerror("Error","Fill all fields",parent=self.root)
        #else:
         #   try:
          #      conn=mysql.connector.connect(host="localhost", username="root" , password="Peehubhatia@123", database="face_recognizer")
           #     my_cursor=conn.cursor()
            #    my_cursor.execute("select * from student")
             #   my_result=my_cursor.fetchall()
              #  id=0
